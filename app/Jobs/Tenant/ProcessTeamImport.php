...
class ProcessTeamImport implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        while (($values = fgetcsv($handle)) !== false) {
            ...
            $team = Team::withoutGlobalScopes()->where('code', trim((string) $row['code']))->first();
            if ($team !== null && ! $this->updateDuplicateRecords) {
        ...
        $message = $errors === []
            ? "Imported {$created} new team record(s) and updated {$updated} existing record(s)."
            : 'Import completed with errors: ' . implode('; ', array_slice($errors, 0, 5));

        $user->notify(new TeamImportExportCompleted('import', $message));
    }
