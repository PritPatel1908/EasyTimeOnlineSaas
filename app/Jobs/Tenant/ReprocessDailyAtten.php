...
class ReprocessDailyAtten implements ShouldBeUnique, ShouldQueue
{
     ...
     */
    public function __construct($users, $dates, $authUser)
    {
        $this->users = $users;
        $this->dates = $dates;
        ...
        foreach ($this->users as $user) {
            foreach ($this->dates as $date) {
                ...
                $holiday = Holiday::where('date', $date->format('Y-m-d'))
                    ->first();
                $curr_att = Attendance::year($date->format('Y'))
                ...
                if (! $curr_att) {
                    if ($week_off !== null && $holiday !== null) {
                        $att = Attendance::year($date->format('Y'))->create([
                            'user_id' => $user->id,
