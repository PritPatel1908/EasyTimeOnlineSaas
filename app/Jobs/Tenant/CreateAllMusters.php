...
class CreateAllMusters implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($users as $user) {
            ...
            if ($alreadyShiftMuster || $alreadyWeekOffMuster) {
                ...
                $this->createStatusMuster($user, $joinDate);
            }
        }
    }
    ...
    public function createShiftMuster($user, $authUser)
    {
        ...
        $from_date = Carbon::parse($shift_change->from_date->copy());
        $fromDate = $from_date->copy();
        if ($shift_change->is_forever) {
            $financial_year = FinancialYear::where('is_started', true)->orderBy('id', 'desc')->first();
            if ($financial_year) {
                $toDate = Carbon::parse($financial_year->end_date);
            } else {
                $toDate = $from_date?->copy()->endOfYear();
            }
        } else {
            $toDate = Carbon::parse($shift_change->to_date);
        }
    ...
    {
        $currentDate = GeneralConfiguration::where('key', 'first_time_muster_date')->first() ? Carbon::parse(GeneralConfiguration::where('key', 'first_time_muster_date')->first()->value) : Carbon::parse($user->join_date);
        $financial_year = FinancialYear::where('is_started', true)->orderBy('id', 'desc')->first();
        if ($financial_year) {
            $toDate = Carbon::parse($financial_year->end_date);
        } else {
        ...
        $from_date = Carbon::parse($week_off_change->from_date->copy());
        if ($week_off_change->is_forever) {
            $financial_year = FinancialYear::where('is_started', true)->orderBy('id', 'desc')->first();
            if ($financial_year) {
                $to_date = Carbon::parse($financial_year->end_date);
            } else {
                $to_date = $from_date?->copy()->endOfYear();
            }
        } else {
            $to_date = Carbon::parse($week_off_change->to_date->copy());
        }
        ...
        if ($week_off_change->is_forever) {
            ...
            if ($financial_year) {
                ...
                $toDate = Carbon::parse($week_off_change->from_date->copy()->endOfYear());
            }
        } else {
            $toDate = Carbon::parse($week_off_change->to_date);
        }

        $currentDate = Carbon::parse($week_off_change->from_date->copy());
        while ($currentDate->lte($toDate)) {
    ...
    public function createStatusMuster($user, $join_date)
    {
        $joinDate = Carbon::parse($join_date);
        ...
        $week_offs = WeekOffMuster::where('user_id', $user->id)->where('is_locked', false)->where('wo_type', 1)->orWhere('wo_type', 2)->orWhere('wo_type', 3)->get();
        if ($week_offs) {
                ...
                if ($week_off->wo_type == 1) {
                    if ($week_off->date < $endDate) {
                        StatusMuster::updateOrCreate(
                            ...
                            [
                                'status_master_id' => $this->allCategoriesHavePaidWeekoff($user) ?
                                    StatusMaster::where('code', 'WO')->first()->id :
                                    StatusMaster::where('code', 'AA')->first()->id,
                                'day_count' => $this->allCategoriesHavePaidWeekoff($user) ? 1.0 : 0.0,
                                'year' => $year,
                            ]
                        );
                    }
        ...
        if ($holidays) {
            foreach ($holidays as $holiday) {
                // Handle both array and single value for location_id and category_id
                $userLocationIds = is_array($user->location_id) ? $user->location_id : [$user->location_id];
                $userCategoryIds = is_array($user->category_id) ? $user->category_id : [$user->category_id];

                // Check if any of the user's locations match holiday locations
                $locationMatch = false;
                foreach ($userLocationIds as $locationId) {
                    if ($holiday->locations->contains('id', $locationId)) {
                        $locationMatch = true;
                        break;
                    }
                }

                // Check if any of the user's categories match holiday categories
                $categoryMatch = false;
                foreach ($userCategoryIds as $categoryId) {
                    if ($holiday->categories->contains('id', $categoryId)) {
                        $categoryMatch = true;
                        break;
                    }
                }

                // If both location and category match, create the status muster
                if ($locationMatch && $categoryMatch && $holiday->date < $endDate) {
                    StatusMuster::updateOrCreate(
                        [
                            'date' => $holiday->date,
                            'user_id' => $user->id,
                        ...
                        [
                            'status_master_id' => $this->allCategoriesHavePaidHoliday($user) ?
                                StatusMaster::where('code', 'PHL')->first()->id :
                                StatusMaster::where('code', 'PH')->first()->id,
                            'day_count' => $this->allCategoriesHavePaidHoliday($user) ? 1.0 : 0.0,
                            'year' => $year,
                        ]
                    );
                }
    ...
    protected function allCategoriesHavePaidHoliday($user)
    {
    ...
    protected function allCategoriesHavePaidWeekoff($user)
    {
        if (! is_array($user->category_id)) {
            $categoryIds = [$user->category_id];
        } else {
