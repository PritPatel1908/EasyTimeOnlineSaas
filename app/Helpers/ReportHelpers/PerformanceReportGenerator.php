...
class PerformanceReportGenerator
{
    ...
    public function generateDetailedXlsxReport($result)
    {
        ...
        $reportTitle = ($companyName ? $companyName.' - ' : '').'Performance Report';
        ...
        if ($hasLogo) {
            ...
            $sheet->mergeCells($titleCell.':'.$this->getColumnLetter(count($data[0])).'1');

            // Add date range and generated date with offset
            $sheet->setCellValue('C2', $dateRange);
            $sheet->mergeCells('C2:'.$this->getColumnLetter(count($data[0])).'2');

            $sheet->setCellValue('C3', $generatedOn);
            $sheet->mergeCells('C3:'.$this->getColumnLetter(count($data[0])).'3');
        } else {
            // Add title row with merge cells (no logo)
            $sheet->setCellValue('A1', $reportTitle);
            $sheet->mergeCells('A1:'.$this->getColumnLetter(count($data[0])).'1');

            // Add date range row
            $sheet->setCellValue('A2', $dateRange);
            $sheet->mergeCells('A2:'.$this->getColumnLetter(count($data[0])).'2');

            // Add generated date row
            $sheet->setCellValue('A3', $generatedOn);
            $sheet->mergeCells('A3:'.$this->getColumnLetter(count($data[0])).'3');
        }
        ...
        if ($hasLogo) {
            $sheet->getStyle('C1:'.$this->getColumnLetter(count($data[0])).'1')->applyFromArray($titleStyle);
            $sheet->getStyle('C2:'.$this->getColumnLetter(count($data[0])).'3')->applyFromArray($dateStyle);
            // Reserve space for logo
            $sheet->getColumnDimension('A')->setWidth(15);
            $sheet->getColumnDimension('B')->setWidth(5);
        } else {
            $sheet->getStyle('A1:'.$this->getColumnLetter(count($data[0])).'1')->applyFromArray($titleStyle);
            $sheet->getStyle('A2:'.$this->getColumnLetter(count($data[0])).'3')->applyFromArray($dateStyle);
        }
        ...
        $sheet->getStyle('A5:'.$this->getColumnLetter(count($data[0])).'5')->applyFromArray($headerStyle);

        // Zebra striping for data rows
        for ($i = 6; $i <= count($data) + 4; $i++) {
            if ($i % 2 == 0) {
                $sheet->getStyle('A'.$i.':'.$this->getColumnLetter(count($data[0])).$i)->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
        ...
        $sheet->getStyle('A5:'.$this->getColumnLetter(count($data[0])).(count($data) + 4))->applyFromArray($borderStyle);
        ...
        $lastColumn = $this->getColumnLetter(count($data[0]));
