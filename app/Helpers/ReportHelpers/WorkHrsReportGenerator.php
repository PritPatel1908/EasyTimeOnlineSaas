...
class WorkHrsReportGenerator
{
    ...
    public function generateWorkHrsReport(): void
    {
        ...
        $attendanceDetails->where('in_time');
    ...
    public function generateDetailedXlsxReport($result)
    {
        ...
        $sheet->setCellValue('A1', 'Working Hours Report');
        ...
        foreach ($datacsv as $row) {
            ...
            if (isset($row[0]) && $row[0] === 'Grand Total') {
                $grandTotalRows[] = $rowIndex;
            }
        ...
        foreach ($grandTotalRows as $row) {
            $sheet->getStyle('A'.$row.':'.$this->getColumnLetter(count($heading)).$row)->applyFromArray($grandTotalStyle);
        }
