...
class AbsentReportGenerator
{
    ...
    public function generateHorizontalCSV($result)
    {
        ...
        foreach ($this->selected_columns as $column) {
            if ($column != 'date') {
                $headerRow[] = AbsentReportColumns::{$column}->getLabel();
            ...
            foreach ($identityColumns as $column) {
                switch ($column) {
                    ...
                    case 'name':
                        ...
                        break;
                    case 'location_name':
                        $row[] = is_array($attendance->user->location_id) ? collect(Location::whereIn('id', $attendance->user->location_id)->get())->pluck('name')->implode(', ') : ($attendance->user->location ? $attendance->user->location->name : '');
                        break;
                    case 'location_code':
                        $row[] = is_array($attendance->user->location_id) ? collect(Location::whereIn('id', $attendance->user->location_id)->get())->pluck('code')->implode(', ') : ($attendance->user->location ? $attendance->user->location->name : '');
                        break;
                    case 'company_name':
                        $row[] = is_array($attendance->user->company_id) ? collect(Company::whereIn('id', $attendance->user->company_id)->get())->pluck('name')->implode(', ') : ($attendance->user->company ? $attendance->user->company->name : '');
                        break;
                    case 'company_code':
                        $row[] = is_array($attendance->user->company_id) ? collect(Company::whereIn('id', $attendance->user->company_id)->get())->pluck('code')->implode(', ') : ($attendance->user->company ? $attendance->user->company->code : '');
                        break;
                    ...
                    case 'sub_category_code':
                        ...
                        break;
                        // case "status":
                        //     // Skip status as it will be per date
                        //     $row[] = "";
                        //     break;
                    default:
                        $row[] = 'Unknown Column';
    ...
    public function generateDetailedXlsxReport($result)
    {
        ...
        $reportTitle = ($companyName ? $companyName.' - ' : '').'Absent Report';
