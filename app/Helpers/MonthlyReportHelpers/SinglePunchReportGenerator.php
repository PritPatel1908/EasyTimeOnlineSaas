...
class SinglePunchReportGenerator
{
    ...
    public function generateSinglePunchReport(): void
    {
        try {
            ...
            $attendanceDetails = Attendance::Year($this->year)
                ->with(['user', 'shift', 'status_master']);

            // Don't eager load relationships with JSON array columns
            ...
            // Get Single Punch - records where user has in_time but no out_time
            $attendanceDetails->whereNotNull('in_time')
                ->whereNull('out_time');
            ...
            // Log the error
            Log::error('Single Punch Report Generation Error: '.$e->getMessage());
            Log::error($e->getTraceAsString());

            // Update report with error status
            $this->monthlyReport->report_status = ReportStatus::error;
            $this->monthlyReport->save();

            // Send notification if possible
            try {
                if ($this->monthlyReport->created_by_user) {
                    $this->monthlyReport->created_by_user->notify(
                        Notification::make()
                            ->title('Report Generation Failed')
                            ->danger()
                            ->icon('heroicon-o-exclamation-circle')
                            ->body('There was an error generating the report: '.$e->getMessage())
                            ->toDatabase(),
                    );
                }
            } catch (\Exception $notificationException) {
                Log::error('Failed to send notification: '.$notificationException->getMessage());
            }
        }
    }
    ...
    public function generateDetailedXlsxReport($result)
    {
        ...
        $reportTitle = 'Single Punch Report';
        ...
        foreach ($this->selected_columns as $column) {
            switch ($column) {
                ...
                case 'name':
                    ...
                    break;
                case 'location_name':
                    if (is_array($attendance->user->location_id)) {
                        $row[] = collect(Location::whereIn('id', $attendance->user->location_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->location ? $attendance->user->location->name : '';
                    }
                    break;
                case 'location_code':
                    if (is_array($attendance->user->location_id)) {
                        $row[] = collect(Location::whereIn('id', $attendance->user->location_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->location ? $attendance->user->location->code : '';
                    }
                    break;
                case 'company_name':
                    if (is_array($attendance->user->company_id)) {
                        $row[] = collect(Company::whereIn('id', $attendance->user->company_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->company ? $attendance->user->company->name : '';
                    }
                    break;
                case 'company_code':
                    if (is_array($attendance->user->company_id)) {
                        $row[] = collect(Company::whereIn('id', $attendance->user->company_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->company ? $attendance->user->company->code : '';
                    }
                    break;
                case 'department_name':
                    if (is_array($attendance->user->department_id)) {
                        $row[] = collect(Department::whereIn('id', $attendance->user->department_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->department ? $attendance->user->department->name : '';
                    }
                    break;
                case 'department_code':
                    if (is_array($attendance->user->department_id)) {
                        $row[] = collect(Department::whereIn('id', $attendance->user->department_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->department ? $attendance->user->department->code : '';
                    }
                    break;
                case 'sub_department_name':
                    if (is_array($attendance->user->sub_department_id)) {
                        $row[] = collect(SubDepartment::whereIn('id', $attendance->user->sub_department_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->sub_department ? $attendance->user->sub_department->name : '';
                    }
                    break;
                case 'sub_department_code':
                    if (is_array($attendance->user->sub_department_id)) {
                        $row[] = collect(SubDepartment::whereIn('id', $attendance->user->sub_department_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->sub_department ? $attendance->user->sub_department->code : '';
                    }
                    break;
                case 'category_name':
                    if (is_array($attendance->user->category_id)) {
                        $row[] = collect(Category::whereIn('id', $attendance->user->category_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->category ? $attendance->user->category->name : '';
                    }
                    break;
                case 'category_code':
                    if (is_array($attendance->user->category_id)) {
                        $row[] = collect(Category::whereIn('id', $attendance->user->category_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->category ? $attendance->user->category->code : '';
                    }
                    break;
                case 'sub_category_name':
                    if (is_array($attendance->user->sub_category_id)) {
                        $row[] = collect(SubCategory::whereIn('id', $attendance->user->sub_category_id)->get())->pluck('name')->implode(', ');
                    } else {
                        $row[] = $attendance->user->sub_category ? $attendance->user->sub_category->name : '';
                    }
                    break;
                case 'sub_category_code':
                    if (is_array($attendance->user->sub_category_id)) {
                        $row[] = collect(SubCategory::whereIn('id', $attendance->user->sub_category_id)->get())->pluck('code')->implode(', ');
                    } else {
                        $row[] = $attendance->user->sub_category ? $attendance->user->sub_category->code : '';
                    }
                    break;
                case 'shift':
