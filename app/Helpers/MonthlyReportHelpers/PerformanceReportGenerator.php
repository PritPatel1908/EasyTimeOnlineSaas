...
use App\Enums\ReportColumns\PerformanceReportColumns;
...
class PerformanceReportGenerator
{
    ...
    private $orderMap = [
        PerformanceReportColumns::date->value => 1,
        PerformanceReportColumns::code->value => 2,
        PerformanceReportColumns::name->value => 3,
        PerformanceReportColumns::location_name->value => 4,
        PerformanceReportColumns::location_code->value => 5,
        PerformanceReportColumns::company_name->value => 6,
        PerformanceReportColumns::company_code->value => 7,
        PerformanceReportColumns::department_name->value => 8,
        PerformanceReportColumns::department_code->value => 9,
        PerformanceReportColumns::sub_department_name->value => 10,
        PerformanceReportColumns::sub_department_code->value => 11,
        PerformanceReportColumns::category_name->value => 12,
        PerformanceReportColumns::category_code->value => 13,
        PerformanceReportColumns::sub_category_name->value => 14,
        PerformanceReportColumns::sub_category_code->value => 15,
        PerformanceReportColumns::shift->value => 16,
        PerformanceReportColumns::shift_in_time->value => 17,
        PerformanceReportColumns::shift_out_time->value => 18,
        PerformanceReportColumns::night_shift->value => 19,
        PerformanceReportColumns::in_time->value => 20,
        PerformanceReportColumns::out_time->value => 21,
        PerformanceReportColumns::holiday->value => 22,
        PerformanceReportColumns::weekoff->value => 23,
        PerformanceReportColumns::late->value => 24,
        PerformanceReportColumns::early->value => 25,
        PerformanceReportColumns::half_day->value => 26,
        PerformanceReportColumns::absent->value => 27,
        PerformanceReportColumns::overtime->value => 28,
        PerformanceReportColumns::working_hrs->value => 29,
        PerformanceReportColumns::manual->value => 30,
        PerformanceReportColumns::has_error->value => 31,
        PerformanceReportColumns::status->value => 32,
        PerformanceReportColumns::remarks->value => 33,
    ];
    ...
    public function generatePerformanceReport(): void
    {
        ...
        // $this->applyMasterFilter($attendanceDetails);
    ...
    public function generateDetailedCsvReport($result)
    {
        ...
        //     $heading[] = PerformanceReportColumns::{$column}->getLabel();
        // }
    ...
    public function renderCSVTree($result)
    {
        ...
        foreach ($result as $employeeId => $records) {
            ...
            $summary = $this->generateEmployeeSummary($records, $all_dates);
            $csv = array_merge($csv, $summary);
            $csv[] = [''];
        }
    ...
    private function generateEmployeeSummary($records, $all_dates)
    {
        // Create a single row for the summary with "SUMMARY:" as the first cell
        $summaryRow = ['SUMMARY:'];

        // Count total days in the period
        $totalDays = count($all_dates);
        $summaryRow[] = "Total Days: $totalDays";

        // Count present days (days with attendance records)
        // $presentDays = $records->count();
        // $summaryRow[] = "Present Days: $presentDays";

        // Count absent days
        // $absentDays = $totalDays - $presentDays;
        // $summaryRow[] = "Absent Days: $absentDays";

        // Count attendance attributes
        $lateCount = $records->where('is_late', true)->count();
        $earlyCount = $records->where('is_early', true)->count();
        $halfDayCount = $records->where('is_half_day', true)->count();
        $holidayCount = $records->where('is_holiday', true)->count();
        $weekoffCount = $records->where('is_weekoff', true)->count();

        $summaryRow[] = "Late Days: $lateCount";
        $summaryRow[] = "Early Going: $earlyCount";
        $summaryRow[] = "Half Days: $halfDayCount";
        $summaryRow[] = "Holidays: $holidayCount";
        $summaryRow[] = "Week Offs: $weekoffCount";

        // Calculate total working hours
        $totalWorkingMinutes = 0;
        $validTimeRecords = $records->filter(function ($record) {
            return $record->in_time && $record->out_time;
        });

        foreach ($validTimeRecords as $record) {
            $totalWorkingMinutes += $record->in_time->diffInMinutes($record->out_time);
        }

        $totalWorkingHours = sprintf('%02d:%02d', floor($totalWorkingMinutes / 60), $totalWorkingMinutes % 60);
        $summaryRow[] = "Total Working Hours: $totalWorkingHours";

        // Add status counts to the same row
        $statusCounts = $records->groupBy(function ($attendance) {
            return $attendance->status_master ? $attendance->status_master->code : 'Unknown';
        })->map->count();

        foreach ($statusCounts as $status => $count) {
            $summaryRow[] = "{$status}: $count";
        }

        // Return a single row as the summary
        return [$summaryRow];
    }
        ...
        foreach ($this->selected_columns as $column) {
            switch ($column) {
                ...
                case 'shift_out_time':
                    ...
                    break;
                case 'night_shift':
                    $row[] = $attendance->is_night_shift ? 'Yes' : '';
                    break;
                case 'in_time':
                ...
                case 'out_time':
                    ...
                    break;
                case 'holiday':
                    $row[] = $attendance->is_holiday ? 'Yes' : '';
                    break;
                case 'weekoff':
                    $row[] = $attendance->is_weekoff ? 'Yes' : '';
                    break;
                case 'late':
                    $late = '';
                    if ($attendance->shift != null) {
                ...
                case 'early':
                    ...
                    break;
                case 'half_day':
                    $row[] = $attendance->is_half_day ? 'Yes' : '';
                    break;
                case 'absent':
                    $row[] = $attendance->is_absent ? 'Yes' : '';
                    break;
                case 'overtime':
                    $overTime = '';
                    if ($attendance->shift != null) {
                ...
                case 'working_hrs':
                    ...
                    break;
                case 'manual':
                    $row[] = $attendance->is_manual ? 'Yes' : '';
                    break;
                case 'has_error':
                    $row[] = $attendance->has_error ? 'Yes' : '';
                    break;
                case 'status':
