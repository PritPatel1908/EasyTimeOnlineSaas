...
use App\Enums\ReportColumns\AbsentReportColumns;
...
class AbsentReportGenerator
{
    ...
    private $orderMap = [
        ...
        LateComingReportColumns::status->value => 16,
    ];
    ...
    public function generateAbsentReport(): void
    {
        ...
        // Get is_late records
        $attendanceDetails->where('status_master_id', StatusMaster::where('code', 'AA')->first()->id);
    ...
    public function generateDetailedCsvReport($result)
    {
        ...
        //     $heading[] = AbsentReportColumns::{$column}->getLabel();
        // }
    ...
    {
        if ($this->monthlyReport->data_format->value === 'horizontal') {
            return $this->renderHorizontalCSVTree($result);
        } else {
            return $this->renderVerticalCSVTree($result);
        }
    }

    public function renderVerticalCSVTree($result)
    {
        $csv = [];
        ...
        foreach ($result as $employeeId => $records) {
            $csv[] = ['User Code : '.$employeeId, 'User Name : '.$records[0]->user->name, 'Location Name : '.is_array($records[0]->user->location_id) ? collect(Location::whereIn('id', $records[0]->user->location_id)->get())->pluck('name')->implode(', ') : ($records[0]->user->location ? $records[0]->user->location->name : '')];
            ...
            foreach ($all_dates as $date) {
                ...
                if ($attendanceRecord) {
                    ...
                    // Create a row with date and dashes for all other columns
                    $emptyRow = [$date->format('d/m/Y')];

                    // Add dashes for all other columns (one less because date is already added)
                    for ($i = 1; $i < count($this->selected_columns); $i++) {
                        $emptyRow[] = '-';
                    }
            ...
            $csv[] = [''];
            $csv[] = ['Monthly Summary', 'Total Absences: '.$totalAbsences];
            $csv[] = [''];
        }
    ...
    public function renderHorizontalCSVTree($result)
    {
        $csv = [];
        $startDate = $this->monthlyReport->from_datetime;
        $endDate = $this->monthlyReport->to_datetime;
        ...
        foreach ($result as $employeeId => $records) {
            $row = [$employeeId, $records[0]->user->name];

            // Add attendance status for each date
            foreach ($all_dates as $date) {
                $attendanceRecord = $records->firstWhere('date', $date);
                if ($attendanceRecord) {
                    $row[] = $attendanceRecord->status_master ? $attendanceRecord->status_master->code : '';
                } else {
                    $row[] = '-';
                }
            }
    ...
    public function renderDetailedCSVColumns($attendance)
    {
        ...
        foreach ($this->selected_columns as $column) {
            switch ($column) {
                case 'date':
                    $row[] = $attendance->date ? $attendance->date->format('d/m/Y') : '';
                    break;
                case 'code':
                    $row[] = $attendance->user ? $attendance->user->code : '';
                    break;
                case 'name':
                    $row[] = $attendance->user ? $attendance->user->name : '';
                    break;
                case 'location_name':
